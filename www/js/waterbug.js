...
var buildForm = function() {
    ...
    if (logoKeys.length > 1) {
        $logosWrapper.append(
            '<div class="btn-group-sm logos" data-toggle="buttons"></div>'
        );
        var $logos = $(".logos");
        for (var j = 0; j < logoKeys.length; j++) {
            ...
            $logos.append(
                '<label class="btn btn-primary"><input type="radio" name="logo" id="' +
                    key +
                    '" value="' +
                    key +
                    '">' +
    ...
    for (var i = 0; i < cropKeys.length; i++) {
        ...
        var tooltip = "";
        if (cropOptions[key].height)
            tooltip = cropOptions[key].width + "x" + cropOptions[key].height;
        $crops.append(
            '<label class="btn btn-primary" title="' +
                tooltip +
                '"><input type="radio" name="crop" id="' +
                key +
                '" value="' +
                key +
                '">' +
...
var renderCanvas = function() {
    ...
    if (cropOptions[currentCrop].height) {
        ...
        ctx.drawImage(img, 0, 0, canvasWidth, scaledImageHeight);
    }
    ...
    var textXY = calcPosition(
        currentTextPosition,
        creditWidth.width,
        parseInt(fontSize),
        true
    );
    ctx.fillText(credit, textXY[0], textXY[1]);
...
var buildCreditString = function() {
    ...
    if ($photographer.val() !== "") {
        ...
        if (copyrightOptions[val]["source"]) {
            creditString =
                $photographer.val() + d + copyrightOptions[val]["source"];
        } else {
...
var onDrag = function(e) {
    ...
    $(document)
        .on("mousemove.drag touchmove", _.debounce(update, 5, true))
...
var handleImage = function(e) {
    ...
    reader.onload = function(e) {
        ...
        imageFilename = $(".fileinput-filename")
            .text()
            .split(".")[0];
        img.src = image;
...
var onSaveClick = function(eve) {
    ...
    console.log(
        "Image download size: " + (link.href.length / 1024).toFixed(2) + "KB"
    );
